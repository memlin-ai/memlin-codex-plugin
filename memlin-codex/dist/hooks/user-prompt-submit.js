...
function companionRunDir() {
  return path5.join(os4.homedir(), ".config", "memlin", "run");
}
...
async function writeWorkspaceBinding(workspaceRoot, binding) {
  ...
  const file = path8.join(dir, WORKSPACE_BINDING_FILE);
  try {
    const existing = await fs5.lstat(file);
  ...
  try {
    ...
    const installed = await fs5.lstat(file);
...
function boundAdditionalContextForHook(additionalContext, options2) {
  const outputBytes = Buffer.byteLength(additionalContext, "utf8");
  if (outputBytes <= options2.maxBytes) {
    return { context: additionalContext, capped: false, outputBytes };
  }
  const context = buildResolveFailedEnvelope({
    message: `Memlin ${options2.label} context exceeded the safe hook envelope; its required content was not delivered.`,
    retryable: true
  });
  const degradedBytes = Buffer.byteLength(context, "utf8");
  if (degradedBytes > options2.maxBytes) {
...
function persistedTokenFilePath() {
  return process.env.MEMLIN_TOKEN_FILE || path6.join(os5.homedir(), ".config", "memlin", "token.json");
}
...
var ClaudeCodeHost = class extends BaseHost {
  constructor() {
    super("claude-code", path7.join(os6.homedir(), ".claude"));
  }
};
var CursorHost = class extends BaseHost {
  constructor() {
    super("cursor", path7.join(os6.homedir(), ".config", "memlin"));
  }
};
var CodexHost = class extends BaseHost {
  constructor() {
    super("codex", path7.join(os6.homedir(), ".config", "memlin"));
  }
};
var WindsurfHost = class extends BaseHost {
  constructor() {
    super("windsurf", path7.join(os6.homedir(), ".config", "memlin"));
  }
};
var AntigravityHost = class extends BaseHost {
  constructor() {
    super("antigravity", path7.join(os6.homedir(), ".config", "memlin"));
  }
};
var VSCodeHost = class extends BaseHost {
  constructor() {
    super("vscode", path7.join(os6.homedir(), ".config", "memlin"));
  }
};
var CompanionHost = class extends BaseHost {
  constructor() {
    super("companion", path7.join(os6.homedir(), ".config", "memlin"));
  }
};
var HOSTS = {
...
function queueFor(state, sessionId) {
  ...
  if (sessionId) return map[sessionId];
  return Object.values(map).sort((a, b) => (b.at ?? 0) - (a.at ?? 0))[0];
}
...
function withPendingDecisions(additionalContext) {
  if (pendingDecisions.length === 0) return additionalContext;
  const decisions = pendingDecisions;
  pendingDecisions = [];
  return composeDecisionContext(decisions, additionalContext, {
    host: "codex",
    maxBytes: CODEX_ADDITIONAL_CONTEXT_MAX_BYTES
  });
}
async function emitAdditionalContext(context, systemMessage = "") {
  const additionalContext = withPendingDecisions(context);
  const outputBytes = Buffer.byteLength(additionalContext, "utf8");
...
async function emitPriorLatePhase(started, input, cwd, beganAt) {
  const late2 = await companionReserveLateResolveDelivery(
    {
      ...waitRequest(started, input, 0),
      owner: "hook",
  ...
  if (late2?.status !== "reserved" || !late2.reservation_token || !late2.phase || late2.phase.type !== "full" || !late2.source) {
    return false;
  }
  const completedAt = Date.parse(late2.phase.completed_at);
  const context = buildLateDeliveryEnvelope(
    {
      resolve_id: late2.source.resolve_id,
      turn_id: late2.source.turn_id,
      trace_id: late2.phase.trace_id,
      task: "",
      cwd,
      host: late2.source.host,
      session_id: late2.source.session_id,
      rendered: late2.phase.rendered ?? "",
      audit_id: late2.phase.audit_id,
      completed_at: Number.isFinite(completedAt) ? completedAt : Date.now(),
      latency_ms: late2.phase.timing_ms ?? 0,
  ...
  return emitClaimedPhase({
    identity: late2.source,
    reservation: {
      ...late2,
      reservation_token: late2.reservation_token,
      phase
    },
    beganAt,
...
async function main() {
  ...
  const decision = mode === "hot" ? await takeSessionDecisions({ sessionId, host: "codex", cwd }) : null;
  pendingDecisions = decision?.decisions ?? [];
  const scribeNotice = mode === "hot" ? await takeCorrectionNotice(sessionId ?? void 0) + await takeScribeNotice(sessionId ?? void 0) : "";
  const scribeSystemMessage = utf8Prefix(
    [decision?.systemMessage ?? "", noticeSystemMessage(scribeNotice)].filter(Boolean).join("\n"),
    SYSTEM_MESSAGE_MAX_BYTES
  );
  try {
    const state = await readState();
