...
async function recordEditActivity(ctx, payload) {
  try {
    ...
    const plansDir = path6.join(os5.homedir(), ".claude", "plans");
    const abs = rawPaths.map((p) => path6.resolve(cwd, p));
    const codePaths = abs.filter((p) => !p.startsWith(plansDir + path6.sep));
...
function statePath(cwd) {
  ...
  return path8.join(os6.tmpdir(), `memlin-codex-heartbeat-${key}.json`);
}
...
async function main() {
  ...
  const abs = file ? path9.resolve(file) : "";
  if (abs && abs.startsWith(plansDir + path9.sep) && abs.endsWith(".md")) {
    try {
