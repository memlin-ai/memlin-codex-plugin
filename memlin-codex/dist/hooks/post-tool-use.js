...
async function recordEditActivity(ctx, payload) {
  try {
    ...
    const plansDir = path7.join(os6.homedir(), ".claude", "plans");
    const abs = rawPaths.map((p) => path7.resolve(cwd, p));
    const codePaths = abs.filter((p) => !p.startsWith(plansDir + path7.sep));
...
function statePath(cwd) {
  ...
  return path9.join(os7.tmpdir(), `memlin-codex-heartbeat-${key}.json`);
}
...
var PUSH_PLAN_BIN = path10.resolve(HOOK_DIR, "../cli/push-plan.js");
function editedFile(input) {
  const ti = input?.tool_input;
  const fp = ti?.file_path;
  const p = ti?.path;
  return typeof fp === "string" && fp || typeof p === "string" && p || "";
}
async function main() {
  ...
  if (ctx && input) {
    ...
    if (input.tool_name === "Bash") {
      ...
      await maybeScribeCommit(ctx, { tool_input: { command }, cwd });
    }
  }
  const file = editedFile(input);
  const plansDir = resolveHost().plansDir();
  const abs = file ? path10.resolve(file) : "";
  if (abs && abs.startsWith(plansDir + path10.sep) && abs.endsWith(".md")) {
    try {
