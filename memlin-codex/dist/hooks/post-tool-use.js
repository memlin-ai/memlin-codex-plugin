...
var PUSH_PLAN_BIN = path6.resolve(HOOK_DIR, "../cli/push-plan.js");
...
async function main() {
  ...
  await recordCodexActivity(process.cwd(), "post-tool-use");
  const file = editedFile(input);
  const plansDir = resolveHost().plansDir();
  const abs = file ? path6.resolve(file) : "";
  if (abs && abs.startsWith(plansDir + path6.sep) && abs.endsWith(".md")) {
    try {
