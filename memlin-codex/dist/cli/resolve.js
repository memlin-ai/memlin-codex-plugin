...
function pendingBundleKey(cwd, host, sessionId, task) {
  return crypto4.createHash("sha256").update(JSON.stringify([cwd, host, sessionId ?? null, task])).digest("hex");
}
function canonicalPendingBundlePathFor(cwd, host, sessionId, task) {
  return path9.join(pendingBundleSpoolDir(), `${pendingBundleKey(cwd, host, sessionId, task)}.json`);
}
function pendingBundleTurnIndexPath(cwd, host, sessionId) {
  const key = crypto4.createHash("sha256").update(JSON.stringify([cwd, host, sessionId ?? null])).digest("hex");
  return path9.join(pendingBundleSpoolDir(), `turn-${key}.json`);
}
...
async function writePendingBundle(bundle) {
  ...
  const canonicalFile = canonicalPendingBundlePathFor(
    bundle.cwd,
    bundle.host,
    bundle.session_id,
    bundle.task
  );
  const isCanonical = path9.resolve(file) === path9.resolve(canonicalFile);
  const parent = path9.dirname(file);
  await fs6.mkdir(parent, { recursive: true, mode: 448 });
  if (isCanonical) await fs6.chmod(parent, 448);
  const tmp = `${file}.${process.pid}.tmp`;
  await fs6.writeFile(tmp, JSON.stringify(bundle), { encoding: "utf8", mode: 384 });
  await atomicRename(tmp, file);
  ...
  if (isCanonical) {
    ...
    let previous = null;
    try {
      const parsed = JSON.parse(await fs6.readFile(indexFile, "utf8"));
      if (/^[a-f0-9]{64}\.json$/.test(parsed.file ?? "")) {
        previous = path9.join(path9.dirname(indexFile), parsed.file);
      }
    } catch {
    }
    ...
    try {
      await fs6.writeFile(
        indexTmp,
        JSON.stringify({ file: path9.basename(file), completed_at: bundle.completed_at }),
        { encoding: "utf8", mode: 384 }
      );
      await atomicRename(indexTmp, indexFile);
      await fs6.chmod(indexFile, 384);
      if (previous && previous !== file) await fs6.rm(previous, { force: true }).catch(() => {
      });
    } catch {
      await fs6.rm(indexTmp, { force: true }).catch(() => {
      });
    }
  }
}
...
function bundleCacheDir() {
  return process.env.MEMLIN_BUNDLE_CACHE_DIR ?? path11.join(os9.homedir(), ".config", "memlin", "bundle-cache");
}
function bundleCacheKey(accountId, projectId) {
  return crypto5.createHash("sha256").update(JSON.stringify([accountId ?? null, projectId ?? null])).digest("hex");
}
function bundleCachePath(accountId, projectId) {
  return path11.join(bundleCacheDir(), `${bundleCacheKey(accountId, projectId)}.json`);
}
...
async function findCachedBundleForWorkspace(cwd, gitRemote) {
  ...
  for (const name of names.filter((n) => n.endsWith(".json")).slice(0, MAX_SCAN_ENTRIES)) {
    const entry = await readEntryFile(path11.join(bundleCacheDir(), name));
    if (!entry) continue;
...
async function main() {
  ...
  const turnId = process.env.MEMLIN_TURN_ID ?? null;
  ...
  const effectiveAccountId = accountOverride ?? config.account_id;
  const resolveId = turnId ? deriveResolveId({ accountId: effectiveAccountId, host, sessionId, turnId }) : null;
  let result;
  try {
      ...
      await api.writeUsageEvent(
        {
          ...
          metadata: {
            outcome: missedDeadline ? "pending" : "inline_full",
            audit_id: result.audit_id ?? null,
            resolve_id: resolveId,
            turn_id: turnId,
            trace_id: null,
            ...
            host,
            project_id: projectId,
            hook_trusted: host === "codex" ? true : null,
            canary_cohort: process.env.MEMLIN_RESOLVE_CANARY_COHORT ?? null,
            companion_used: false,
            fallback_path: "detached_cli_spool",
            full_output_bytes: Buffer.byteLength(outString),
            client_elapsed_ms: latencyMs
          }
        },
        accountOverride ? { accountId: accountOverride } : {}
      );
